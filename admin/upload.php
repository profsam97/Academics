...
***********************************************************/
	if (!isset($_POST['x']) && !isset($_FILES['image']['name']) ){
		//Delete users temp image
			$temppath = '/movilla/icons/'.$profile_id.'_temp.jpeg';
...
***********************************************************/	
	...
	//Set Upload directory    
		$uploaddir = $_SERVER['DOCUMENT_ROOT'].'/movilla/icons';
	//Set File name	
		$file_temp_name = $profile_id.md5(time()).'n'.$type; //the temp file name
		$fullpath = $uploaddir."/".$file_temp_name; // the temp file path
		$file_name = $profile_id.md5(time()).'n.jpeg'; //$profile_id.'_temp.'.$type; // for the final resized image
		$fullpath_2 = $uploaddir."/".$file_name; //for the final resized image
		$result_paths ="icons/".$profile_id.md5(time())."n.jpeg";
	//Move the file to correct location
		...
		if (!$move) { 
			...
			$imgSrc= "/movilla/icons/".$file_name; // the image to display in crop area
			$msg= "Upload Complete!";  	//message to page
			$src = $file_name;	 		//the file name to post from cropping form to the resize		
		} 
		$insert_pic_query = mysqli_query($connection, "UPDATE users SET user_image='$result_paths' WHERE username='$userLoggedIn'");
		confirmQuery($insert_pic_query);
...
if (isset($_POST['x'])){
	...
	if($type == 'jpg' || $type == 'jpeg' || $type == 'JPG' || $type == 'JPEG'){	
		...
		//save the new cropped version
			imagejpeg($dst_r, "../icons/".$finalname."n.jpeg", 90); 	
		...
		//create a cropped copy of the image
			...
			$targ_w,$targ_h,$_POST['w'],$_POST['h']);
		//save the new cropped version
			imagejpeg($dst_r, "/movilla/icons/".$finalname."n.jpeg", 90); 	
		...
		$result_path ="icons/".$finalname."n.jpeg";

		//Insert image into database
		$insert_pic_query = mysqli_query($connection, "UPDATE users SET user_image='$file_name' WHERE username='$userLoggedIn'");
		confirmQuery($insert_pic_query);
		header("Location: ../user/user_profile.php");
...
<div class="main_column column">
	...
	<div id="formExample">
	    ...
	    <form action="upload.php" class="form-group m-5 p-4" method="post"  enctype="multipart/form-data">
			<div class="form-group">
                <div class="form-group">
                  ...
                  <small id="fileHelpId" class="form-text text-muted">Max size 3mb</small>
				</div>
    ...
    if($imgSrc){ //if an image has been uploaded display cropping area?>
	    ...
	    <div id="CroppingContainer" style="width:800px; max-height:600px; background-color:#FFF; margin-left: -200px; position:relative; overflow:hidden; border:2px #666 solid; z-index:2001; padding-bottom:0px;">  
	        ...
	        <div id="InfoArea" class="mx-4" style="width:180px; height:150px; position:relative; overflow:hidden;">	
