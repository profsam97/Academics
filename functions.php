...
function profilePic(){
    if(isLoggedIn()){
        ...
        return mysqli_num_rows($result) >= 1 ? $profile['user_image'] : false; 
    }
    return false;
}
...
function insert_unit(){
    ...
    $stmt = mysqli_prepare($connection, "INSERT INTO unit(unit_title) VALUES(?) ");
...
function deleteUnit(){
    global $connection;
        ...
        if(isset($_GET['delete'])){
        $the_cat_id = $_GET['delete'];
        $query = "DELETE FROM unit WHERE unit_id = {$the_cat_id} ";
        $delete_query = mysqli_query($connection,$query);
...
function register_user($username, $email, $password, $images,$role){
    ...
    global $connection;
        ...
        $query .= "VALUES('{$username}','{$email}',  '{$password}', '{$images}' , '{$role}' )";
 ...
 function updateProfile($username, $new, $old, $image){
        global $connection;
        $new = trim($new);
        $old = trim($old);
        if($image != ""){
        ...
        $old = mysqli_real_escape_string($connection,$old);
      ...
      if(password_verify($old, $password)){
        $up_query = "UPDATE users SET user_password = '{$new}'  WHERE username ='{$username}' ";
        $select_user_update = mysqli_query($connection, $up_query);
        confirmQuery($select_user_update);
        echo "<div class='success re'></div>";
    } 
        else if (password_verify($old, $new) && $image !=""){
            $up_query = "UPDATE users  SET user_password = '{$new}', user_image = '{$image}',  WHERE username ='{$username}'";
            $select_user_update = mysqli_query($connection, $query);
            echo "<div class='success re'></div>";
        }
        else{
            echo "<div class='error re'></div>";
        }
     ...
     while ($row = mysqli_fetch_array($select_user_query)) {
         ...
         if (password_verify($password,$db_user_password)) {
            ...
            header($_SERVER['HTTP_HOST']);
             ...
             redirect("user/user_profile.php");
